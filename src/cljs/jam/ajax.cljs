;; custom version of http-fx that contains the url in the success response
(ns jam.ajax
  (:require
...
(defn ajax-xhrio-handler
  ...
  (if success?
    (on-success {:response response :uri (.getLastUri xhrio)})
    (let [details (merge
...
(reg-fx :http-xhrio-uri http-effect)
